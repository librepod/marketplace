...
apps** — those are system apps, not user-installable. The catalog file is generated by CI from `apps/*/metadata.yaml` and shipped as a
cosign-signed OCI artifact; a dedicated Flux Kustomization (`marketplace-catalog`, defined
in `infrastructure/system-apps/`) renders the mounted ConfigMap and updates it **in place**,
so the `fs.watch` reload below picks up new apps without a restart. Locally regenerate with
`bash ./scripts/generate-catalog.sh` (root output is gitignored); e2e uses fixtures.
